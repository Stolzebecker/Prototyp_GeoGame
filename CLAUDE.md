...
**Backend:** `apps-script/Code.gs`, analog zum Vorbild in `03_Projektphasen/02_Empirische Datenerhebung/TIERList/apps-script/Code.gs`, aber mit eigenem Google Sheet + eigenem `SUBMIT_TOKEN`. Sechs Tabs, verknüpft über `participant_id`/`lauf_id` (siehe Code.gs-Kommentare für die genaue `resolveRun_`-Logik):
