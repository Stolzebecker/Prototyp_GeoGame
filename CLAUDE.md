...
**Backend:** `apps-script/Code.gs`, analog zum Vorbild in `03_Projektphasen/02_Empirische Datenerhebung/TIERList/apps-script/Code.gs`, aber mit eigenem Google Sheet + eigenem `SUBMIT_TOKEN`. Sieben Tabs, verknüpft über `participant_id`/`lauf_id` (siehe Code.gs-Kommentare für die genaue `resolveRun_`-Logik):
- **Personendaten** — 1 Zeile pro `participant_id`, nur beim allerersten Formular (Platzhalter-Felder: Alter, Bildungsabschluss, Studienfach, GIS-Erfahrung, Geschlecht, Gerät [Computer/Tablet/Handy, seit 2026-08-26 — `geraet`-Spalte per `ensureColumn_()` selbstheilend angehängt, siehe `handlePerson_`] — Feldliste noch nicht literaturbasiert final).
...
**Level count (reduced 2026-08-26):** `data/config.json` now has 9 entries (`B1`–`B9`) instead of 15 — `B1` stays the tutorial's silent practice round, `B2`–`B9` are the 8 real gameplay levels (was 14, `B10`–`B15` dropped). This was a pure `config.json` trim — the underlying `Bilder/EO_Bilder/B10norm.png`…`B15norm.png`, `Bilder/Hitboxes/B10puf.geojson`…`B15puf.geojson`, and `input/10.tif`…`15.tif` were deliberately left on disk (not deleted) so the cut levels can be reinstated later by just re-adding their config entries. The freed-up playtime was reinvested in the Post-Befragung (see Datenerhebung above).

**Randomised level order:** `buildLevelOrder()` shuffles indices `1..N-1` into the global `levelOrder` (index `0` is reserved for the tutorial's silent preload/practice round and never appears in the timed experiment). `orderPos` tracks the current position in that shuffled sequence; `nextLevel()` increments `orderPos` and loads `levelOrder[orderPos]`, or calls `finishExperiment()` once exhausted (since 2026-08-26 this runs the Familiarity-Check before `showResults()` — see "Familiarity-Check" under Datenerhebung above; `debugJumpToResults()` bypasses it and calls `showResults()` directly). The top progress bar/text and the "Bereit" overlay both display `orderPos+1 / levelOrder.length`, **not** the raw `CONFIG.levels` index — so progress numbering reflects play order, not image order. Debug-panel level navigation (`debugPrevLevel`/`debugNextLevel`) deliberately bypasses this and steps through raw config indices instead, for QA purposes.
